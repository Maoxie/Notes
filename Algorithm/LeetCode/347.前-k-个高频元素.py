...
第二步，将每个整数出现的次数构建一个大根堆，依次取出堆的前k个元素。
"""
...
class BigRootHeap:
    ...
    def get_parent_rank(self, rank: int):
        """获取父节点"""
        if rank > 0:
            return (rank - 1) // 2
        else:
    ...
    def get_left_child_rank(self, rank: int):
        """获取左子节点"""
        return rank * 2 + 1

    def get_right_child_rank(self, rank: int):
        """获取右子节点"""
        return rank * 2 + 2
